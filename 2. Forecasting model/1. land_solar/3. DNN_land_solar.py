...
def loading_model(load_model=True):  # True is loading weight, False is fitting and saving model
    if load_model:
        model.load_weights(f'./save_model/DNN/DNN_land_solar{i}.h5')
    else:
        # Trains the model for a given number of epochs (iterations on a dataset).
        model.fit(train_x, train_y, epochs=20, batch_size=18, validation_split=0.2, verbose=2)
        model.save_weights(f"./save_model/DNN/DNN_land_solar{i}.h5")
...
for i in range(V.shape[0]):
    ...
    # Sequential model
    model = Sequential()
    model.add(Dense(64, input_shape=(look_back, nFeature), activation='relu'))
...
df.to_csv('.\\result\DNN_land_solar.csv')
...
df1.to_csv('.\\result\DNN_land_solar_performance.csv')
