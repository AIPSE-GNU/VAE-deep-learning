...
def loading_model(load_model=True): # True is loading weight, False is fitting and saving model
	if load_model:
		model.load_weights(f'./save_model/LSTM/LSTM_land_solar{i}.h5')
	else:
		# Trains the model for a given number of epochs (iterations on a dataset).
		model.fit(train_x, train_y, epochs=20, batch_size=18, validation_split= 0.2, verbose=2)
		model.save_weights(f"./save_model/LSTM/LSTM_land_solar{i}.h5")
...
nHidden = 50 # number of hidden layer
nFeature = 1  # Features, columns
shift = 24 # How many shift data
...
# Load Data
filename = 'sampling_land_solar_50.csv'
...
for i in range(V.shape[0]):
	...
	# Sequential model
	model = Sequential()
	model.add(LSTM(nHidden, input_shape = (look_back, nFeature)))
	model.add(Dense(1))
...
df.to_csv('.\\result\LSTM_land_solar.csv')

# Save evaluation CSV file
ind = ['r2_train', 'r2_test', 'rmse_train', 'rmse_test', 'mae_train', 'mae_test', 'mape_train', 'mape_test']
df1 = np.concatenate((txt_r2_train, txt_r2_test, txt_rmse_train, txt_rmse_test, txt_mae_train, txt_mae_test,
					  txt_mape_train, txt_mape_test), axis=1)
df1 = df1.T
df1 = pd.DataFrame(df1, index=ind)
df1.to_csv('.\\result\LSTM_land_solar_performance.csv')